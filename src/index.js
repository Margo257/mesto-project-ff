...
function deleteCard(initialCard, cardEl) {
  cardToDelete = initialCard;
  cardElementToDelete = cardEl;
  openPopup(popupDelete);
}

function submitDeleteCard(initialCard, cardEl) {
  deleteCardRequest(initialCard)
    .then(() => {
      cardEl.remove();
...
formDelete.addEventListener('submit', (evt) => {
  evt.preventDefault();
  submitDeleteCard(cardToDelete, cardElementToDelete);
});
...
function handleFormAddCardSubmit(evt) {
  ...
  sendCardInfo(newCard)
    .then((res) => {
      placesList.prepend(
        createCard(res, deleteCard, like, openImg, res.owner._id)
      );
